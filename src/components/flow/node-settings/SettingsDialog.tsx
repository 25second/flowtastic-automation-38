...
export const SettingsDialog = ({
  ...
  const handleOutputToggle = (outputId: string) => {
    const newOutputs = selectedOutputs.includes(outputId) ? selectedOutputs.filter(id => id !== outputId) : [...selectedOutputs, outputId];
  ...
  const renderOutputSelectors = () => {
    ...
    return <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {availableOutputs.map(output => <div key={output.id} className="flex items-center space-x-2">
              <Checkbox id={output.id} checked={selectedOutputs.includes(output.id)} onCheckedChange={() => handleOutputToggle(output.id)} />
  ...
  const renderSettingInput = (key: string, value: any) => {
    if (nodeData.type === 'generate-person') {
      if (key === 'emailDomain') {
        return <SettingInput key={key} settingKey={key} value={value} localSettings={localSettings} onSettingChange={handleSettingChange} isEmailDomain={true} />;
      }
      if (key in settingOptions) {
        return <SettingInput key={key} settingKey={key} value={value} localSettings={localSettings} onSettingChange={handleSettingChange} options={settingOptions[key as keyof typeof settingOptions]} />;
      }
    }
    if (key !== 'selectedOutputs') {
      return <SettingInput key={key} settingKey={key} value={value} localSettings={localSettings} onSettingChange={handleSettingChange} />;
    }
    return null;
  };
