...
export const getPromptTemplates = () => {
  ...
  const TASK_PLANNING_TEMPLATE = (task: string) => `
  ...
  const ACTION_DECISION_TEMPLATE = (step: string, state: AgentState) => `
  ...
  const VISUAL_ANALYSIS_TEMPLATE = (step: string, state: AgentState, screenshot: string) => `
  Current step to execute: ${step}
